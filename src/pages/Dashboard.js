...
function Dashboard() {
  ...
  const { menuSetupComplete, loading: menuSetupCompleteLoading } = useSelector(
    (state) => state.admin,
  );
  console.log("menuSetupComplete_", menuSetupComplete);
  ...
  useEffect(() => {
    ...
    dispatch(fetchAdminDetails({ adminIdLocal, app }));
    dispatch(fetchSetupGuideWarning({ adminIdLocal, app }));
  ...
  useEffect(() => {
    if (
      menuSetupComplete &&
      (!menuSetupComplete.appEnabled ||
        (menuSetupComplete.missingMenus &&
          menuSetupComplete.missingMenus.length > 0))
    ) {
      setWarningData(menuSetupComplete);
    }
  }, [menuSetupComplete]);
  console.log("warningData_", warningData);
  const showWarning =
    warningData &&
    (!warningData.appEnabled || warningData.missingMenus?.length > 0);

  return (
    <>
      {app && <TitleBar title="" />}
      <Page title="Consultly">
          ...
          <BlockStack gap="200">
            {showWarning && (
              <Banner title="Setup Required" tone="warning">
                ...
                {/* 👈 spacing control */}
                {/* App not enabled */}
                {!warningData.appEnabled && (
                  <Text as="p" tone="critical">
                    ⚠️ App is not activated
                  </Text>
                )}
                {/* Missing menus */}
                {warningData.missingMenus?.length > 0 && (
                  <BlockStack gap="100">
                    <Text as="p">Missing menu links:</Text>

                    <List>
                      {warningData.missingMenus.map((menu, index) => (
                        <List.Item key={index}>{menu}</List.Item>
                      ))}
                    </List>
                  </BlockStack>
                )}
          ...
          {isBannerVisible && (
            <Layout.Section>
              ...
              >
                <BlockStack gap="200">
                  <p>
                    Manage your consultants efficiently and track their
                    performance.
                  </p>
                  <p>
                    Add new consultants, update their status, and monitor
                    consultations.
                  </p>
                </BlockStack>
              </Banner>
                  ...
                  <Box padding="400">
                    ...
                    >
                      ...
                      <div style={{ flex: 1 }}>
                        <Text variant="headingLg" as="h2" fontWeight="bold">
                          {Number(
                            adminDetails_?.adminPersenTage?.$numberDecimal || 0,
                          )}
                          %
                        </Text>
                    ...
                    >
                      ...
                      <div style={{ flex: 1 }}>
                        <Text variant="headingLg" as="h2" fontWeight="bold">
                          ...
                          {formatAmountHelper(
                            adminDetails?.adminWalletBalance?.$numberDecimal ||
                              0,
                          )}
                        </Text>
                        <Text variant="bodyMd" as="p" tone="subdued">
